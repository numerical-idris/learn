...
--}
||| The `Tensor` API. A `Tensor` is an array (or scalar) of numbers or booleans. learn tracks
...
|||
||| learn achieves efficient reuse of tensor computations with `Tag`. See the tutorial
...
interface Taggable a where
  ...
  |||
  ||| See tutorial [_Nuisances in the Tensor API_](https://github.com/numerical-idris/learn/blob/master/tutorials/Nuisances.md) for details.
